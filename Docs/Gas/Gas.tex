...
(\beta_7)^n_{m+\cutefrac{3}{2}} (u_{m+2} - u_{m})\\
...
(\beta_7)^{n-1}_{m+\cutefrac{3}{2}} (\check{u}_{m+2} - \check{u}_{m})\\
...
F^n_{m+\half} &= \frac{1}{2}(F^n_m + F^n_{m+1}) - \frac{1}{2}|A|(u^{n}_{m+1} - u^{n}_{m})\\
F^n_{m-\half} &= \frac{1}{2}(F^n_m + F^n_{m-1}) - \frac{1}{2}|A|(u^{n}_{m} - u^{n}_{m-1}).
\end{align*}

Заменой $\beta_j \rightarrow \Omega \mathrm{B}_j \Omega^{-1}, \quad \gamma_j \rightarrow \frac{\tau}{h}\Omega \Gamma_j \Omega^{-1}$ 
...
Полученная система на верхнем слое решается либо прогонкой
